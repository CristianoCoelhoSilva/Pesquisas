...
base = sqldf("select ano_obito ano, idade_obito, causabas_capitulo, def_sexo sexo from base where causabas_capitulo like '%II.  Neoplasias (tumores)%'")
...
feminino =  sqldf("select faixa,((sum(total)*1000) / 235290) total  from feminino group by faixa")
...
masculino =  sqldf("select faixa,((sum(total) * 1000) / 235290) total  from masculino group by faixa")

#Neoplastia
xy.pop<-c(0.1,0.1,0.1,0.2,0.2,0.3,0.4,0.6,1.0,1.8,3.2,5.0,6.6,7.4,7.3,6.5,5.4,5.4)
xx.pop<-c(0.1,0.1,0.1,0.1,0.2,0.3,0.7,1.1,1.8,2.3,3.5,4.6,5.5,5.9,5.6,5.1,4.4,5.4)
