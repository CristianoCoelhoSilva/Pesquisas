...
base = sqldf("select ano_obito ano, idade_obito, causabas_capitulo, def_sexo sexo from base where causabas_capitulo like '%X.   Doenças do aparelho respiratório%'")
...
feminino =  sqldf("select faixa,((sum(total)*1000) / 161994) total  from feminino group by faixa")
...
masculino =  sqldf("select faixa,((sum(total) * 1000) / 161994) total  from masculino group by faixa")

#Respiratorio
xy.pop<-c(0.8,0.0,0.1,0.1,0.2,0.3,0.4,0.6,0.8,1.1,1.6,2.5,3.6,4.6,5.8,6.8,7.4,13.0)
xx.pop<-c(0.7,0.0,0.0,0.1,0.1,0.1,0.2,0.3,0.4,0.6,1.1,1.7,2.7,3.6,4.6,6.0,7.7,18.5)
